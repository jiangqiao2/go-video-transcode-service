...
func NewHLSWorker(id string, hlsRepo repo.HLSJobRepository, hlsService service.HLSService, storage gateway.StorageGateway, workerCount int) HLSWorker {
    if workerCount <= 0 { workerCount = 1 }
    return &hlsWorkerImpl{ id: id, hlsRepo: hlsRepo, hlsService: hlsService, storage: storage, workerCount: workerCount, stats: WorkerStats{ StartTime: time.Now() } }
}

func (w *hlsWorkerImpl) Start(ctx context.Context) error {
    w.mu.Lock(); defer w.mu.Unlock()
    if w.running { return fmt.Errorf("worker %s is already running", w.id) }
...
func (w *hlsWorkerImpl) workerLoop(ctx context.Context) {
    ...
    for {
        ...
        default:
            ...
            if len(jobs) == 0 { time.Sleep(2 * time.Second); continue }
            job := jobs[0]
...
func (w *hlsWorkerImpl) processJob(ctx context.Context, job *entity.HLSJobEntity) {
    ...
    _ = filepath.WalkDir(base, func(path string, d os.DirEntry, err error) error {
        ...
        rel := path
        if strings.HasPrefix(rel, "storage"+string(filepath.Separator)) { if r, e := filepath.Rel("storage", rel); e == nil { rel = r } }
    ...
    if master != nil {
        m := *master
        if strings.HasPrefix(m, "storage"+string(filepath.Separator)) { if r, e := filepath.Rel("storage", m); e == nil { m = r } }
        masterKey = filepath.ToSlash(m)
    }
    if masterKey != "" { _ = w.hlsRepo.UpdateHLSJobOutput(ctx, job.JobUUID(), masterKey) }
...
func (w *hlsWorkerImpl) updateStats(f func(*WorkerStats)) { w.mu.Lock(); defer w.mu.Unlock(); f(&w.stats) }
