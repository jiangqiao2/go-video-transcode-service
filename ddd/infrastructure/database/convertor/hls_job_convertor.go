...
func (c *HLSJobConvertor) ToEntity(poJob *po.HLSJob) *entity.HLSJobEntity {
	...
	if poJob.ProfilesJSON != nil {
		_ = cfg.FromJSON(*poJob.ProfilesJSON)
	}
	...
	if poJob.MasterPlaylist != nil {
		cfg.SetOutputPath(*poJob.MasterPlaylist)
	}
...
func (c *HLSJobConvertor) ToPO(e *entity.HLSJobEntity) *po.HLSJob {
	var profiles *string
	if e.GetConfig() != nil {
		if json, err := e.GetConfig().ToJSON(); err == nil {
			profiles = &json
		}
	}
	return &po.HLSJob{
