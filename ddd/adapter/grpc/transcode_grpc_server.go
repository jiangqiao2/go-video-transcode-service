...
func (s *TranscodeGrpcServer) CreateTranscodeTask(ctx context.Context, req *transcodepb.CreateTranscodeTaskRequest) (*transcodepb.CreateTranscodeTaskResponse, error) {
	// 检查应用层是否初始化
	if s.app == nil {
		logger.WithContext(ctx).Errorf("transcode app not initialised for gRPC server")
	...
	if userUUID == "" {
		logger.WithContext(ctx).Warnf("CreateTranscodeTask called with empty user_uuid")
		return &transcodepb.CreateTranscodeTaskResponse{
			Success: false,
	...
	if videoUUID == "" {
		logger.WithContext(ctx).Warnf("CreateTranscodeTask called with empty video_uuid user_uuid=%s", userUUID)
		return &transcodepb.CreateTranscodeTaskResponse{
			Success: false,
	...
	if inputPath == "" {
		logger.WithContext(ctx).Warnf("CreateTranscodeTask called with empty input_path user_uuid=%s video_uuid=%s", userUUID, videoUUID)
		return &transcodepb.CreateTranscodeTaskResponse{
			Success: false,
	...
	logger.WithContext(ctx).Infof("CreateTranscodeTask called user_uuid=%s video_uuid=%s input_path=%s target_resolution=%s target_bitrate=%s", userUUID, videoUUID, inputPath, req.GetTargetResolution(), req.GetTargetBitrate())
	...
	if err != nil {
		logger.WithContext(ctx).Errorf("failed to create transcode task user_uuid=%s video_uuid=%s error=%v", userUUID, videoUUID, err)
	...
	logger.WithContext(ctx).Infof("transcode task created successfully task_uuid=%s user_uuid=%s video_uuid=%s", taskDto.TaskUUID, userUUID, videoUUID)
...
func (s *TranscodeGrpcServer) GetTranscodeTask(ctx context.Context, req *transcodepb.GetTranscodeTaskRequest) (*transcodepb.GetTranscodeTaskResponse, error) {
	...
	if taskUUID == "" {
		logger.WithContext(ctx).Warnf("GetTranscodeTask called with empty task_uuid")
		return &transcodepb.GetTranscodeTaskResponse{
			Success:      false,
	...
	logger.WithContext(ctx).Infof("GetTranscodeTask called task_uuid=%s", taskUUID)

	// 调用应用层服务
	taskDto, err := s.app.GetTranscodeTask(ctx, taskUUID)
	if err != nil {
		if err == errno.ErrTranscodeTaskNotFound {
			logger.WithContext(ctx).Warnf("transcode task not found task_uuid=%s", taskUUID)
			return &transcodepb.GetTranscodeTaskResponse{
				Success:      false,
		...
		logger.WithContext(ctx).Errorf("failed to get transcode task task_uuid=%s error=%v", taskUUID, err)
		return &transcodepb.GetTranscodeTaskResponse{
			Success:      false,
	...
	logger.WithContext(ctx).Infof("transcode task retrieved successfully task_uuid=%s video_uuid=%s status=%s progress=%d", taskDto.TaskUUID, taskDto.VideoUUID, taskDto.Status, progress)
