...
func (e *HLSJobEntity) SetStatus(status vo.HLSStatus) { e.status = status.String(); e.updatedAt = time.Now() }
func (e *HLSJobEntity) SetProgress(p int) { if p < 0 { p = 0 } ; if p > 100 { p = 100 } ; e.progress = p ; e.updatedAt = time.Now() }
func (e *HLSJobEntity) SetMasterPlaylist(path string) { e.masterPlaylist = &path ; e.updatedAt = time.Now() }
...
func (e *HLSJobEntity) SetSource(jobUUID *string, sourceType string) { e.sourceJobUUID = jobUUID ; e.sourceType = sourceType ; e.updatedAt = time.Now() }